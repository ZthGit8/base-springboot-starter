...
public class ExcelExporter {
     ...
     * @param sheetName
     * @param dataList
     * @param cellDefinitionConfigurer
     * @return
     * @param <Entity>
     */
    public static <Entity> String export(String fileName, String sheetName, List<Entity> dataList, CellDefinitionConfigurer<Entity> cellDefinitionConfigurer) {
        if (StringUtils.isBlank(sheetName)) {
            sheetName = fileName;
        }
        // 获取列表定义
        List<CellDefinition<Entity>> cellDefinitionList = cellDefinitionConfigurer.getCellDefinitionList();
        if (CollectionUtil.isEmpty(cellDefinitionList)) {
            logger.warn("导出excel配置为空");
            return "";
        }
        // 表格数据
        try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            exportExcel(fileName, sheetName, dataList, cellDefinitionList, out);
     ...
     * @param <Entity>                 泛型实体类
     */
    public static <Entity> void exportOutPutStream(String fileName, String sheetName, List<Entity> dataList, CellDefinitionConfigurer<Entity> cellDefinitionConfigurer, HttpServletResponse response) {
        // 如果工作表名为空，则使用文件名作为工作表名
        if (StringUtils.isBlank(sheetName)) {
            sheetName = fileName;
        }
        // 获取列表定义
        ...
        try {
            ...
            // 直接写入到响应流
            exportExcel(fileName, sheetName, dataList, cellDefinitionList, outs);
            // 刷新输出流
            outs.flush();
        } catch (Exception e) {
            // 记录导出错误日志
     ...
     *
     * @param fileName           文件名，用于生成Excel文件的名称
     * @param sheetName          工作表名，用于Excel工作表的标题
     * @param dataList           数据列表，包含要导出的所有数据实体
     * @param cellDefinitionList 单元格定义列表，指定了表格的列以及如何从数据实体中获取数据
     * @param outputStream       输出流
     * @throws Exception 异常
     */
    private static <Entity> void exportExcel(String fileName, String sheetName, List<Entity> dataList,
                                                List<CellDefinition<Entity>> cellDefinitionList, OutputStream outputStream) throws Exception {
        // 如果工作表名为空，则使用文件名作为工作表名
        if (StringUtils.isBlank(sheetName)) {
            sheetName = fileName;
        }
        ...
        try (SXSSFWorkbook workbook = new SXSSFWorkbook(10000)) {
            ...
            if (cellDefinitionList.stream().allMatch(c -> c.getColumnWidth() > 0)) {
                List<List<CellInfo>> cellInfoList = dataList.stream().map(data -> {
                    ...
                    for (CellDefinition<Entity> cellDefinition : cellDefinitionList) {
                        cellInfos.add(new CellInfo(cellDefinition.getName(), cellDefinition.getValue().apply(data), cellDefinition.getColumnWidth()));
                    }
                    return cellInfos;
                }).toList();
                ...
                List<Map<String, Object>> exportDataList = dataList.stream()
                        .map(data -> {
                            ...
                            for (CellDefinition<Entity> cellDefinition : cellDefinitionList) {
                                ...
                                rowMap.put(cellDefinition.getName(), cellDefinition.getValue().apply(data));
                            }
                            return rowMap;
                        })
