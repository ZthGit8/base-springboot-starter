...
public class AssertUtil {
    ...
    public static <T> void fastFailValidate(T obj) {
        ...
        if (constraintViolations.size() > 0) {
            throwException(ResultCode.PARAM_ERROR,constraintViolations.iterator().next().getMessage());
        }
    }
    ...
    public static <T> void allCheckValidateThrow(T obj) {
        ...
        if (constraintViolations.size() > 0) {
            ...
            //去掉最后一个逗号
            throwException(ResultCode.PARAM_ERROR, errorMsg.toString().substring(0, errorMsg.length() - 1));
        }
    }
     ...
     * @param msg        异常消息
     * @throws BusinessException 如果表达式为假，抛出业务异常
     */
     ...
     * @param args
     */
    public static void isTrue(boolean expression, ResultCode resultCode, Object... args) {
        if (!expression) {
            throwException(resultCode, args);
        }
    }
    ...
    public static void isFalse(boolean expression, ResultCode resultCode, Object... args) {
        if (expression) {
            throwException(resultCode, args);
        }
    }
    ...
    public static void isNotEmpty(Object obj, ResultCode resultCode, Object... args) {
        if (isEmpty(obj)) {
            throwException(resultCode, args);
        }
    }
    ...
    private static void throwException(ResultCode resultCode, Object... arg) {
        if (Objects.isNull(resultCode)) {
            resultCode = ResultCode.INTERNAL_SERVER_ERROR;
        }
        throw new BusinessException(resultCode.getCode(), MessageFormat.format(resultCode.getMessage(), arg));
    }
