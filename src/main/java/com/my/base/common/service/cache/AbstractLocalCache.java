...
public abstract class AbstractLocalCache<IN, OUT> implements BatchCache<IN, OUT> {
    ...
    private void init(long refreshSeconds, long expireSeconds, int maxSize) {
        ...
        this.outClass = (Class<OUT>) type2Class(genericSuperclass.getActualTypeArguments()[1]);
        this.inClass = (Class<IN>) type2Class(genericSuperclass.getActualTypeArguments()[0]);
        cache = Caffeine.newBuilder()
                ...
                .build(new CacheLoader<>() {
                    ...
                    @Override
                    public Map<IN, OUT> loadAll(Set<? extends IN> keys) {
                        //  keys 转为数组
                        IN[] ins = keys.toArray((IN[]) Array.newInstance(inClass, keys.size()));
