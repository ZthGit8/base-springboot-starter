...
import com.my.base.shared.annotation.FrequencyControl;
import com.my.base.features.ratelimit.strategy.FrequencyControlConstant;
import com.my.base.features.ratelimit.domain.FixedWindowDTO;
import com.my.base.features.ratelimit.domain.SlidingWindowDTO;
...
import com.my.base.features.ratelimit.service.FrequencyControlInvoke;
import com.my.base.web.context.RequestContext;
import com.my.base.shared.util.SpElUtil;
...
public class FrequencyControlAspect {
     ...
     */
    @Around("@annotation(com.my.base.shared.annotation.FrequencyControl) || @annotation(com.my.base.shared.annotation.FrequencyControlContainer)")
    ...
    private String getKey(Method method, ProceedingJoinPoint joinPoint, FrequencyControl annotation) {
        switch (annotation.target()) {
            case EL:
                try {
                    // 解析SpEl表达式
                    return SpElUtil.parseSpEl(method, joinPoint.getArgs(), annotation.spEl());
                } catch (IllegalAccessException e) {
                    throw new RuntimeException("Failed to parse SpEl expression", e);
                }
            case IP:
                return RequestContext.getRequestInfo().getRequestIp(); // 获取请求IP
            default:
                return "";
        }
    }
    ...
    private FrequencyControlDTO createDTOTemplate(FrequencyControl annotation) {
        switch (annotation.strategy()) {
            ...
            case FrequencyControlConstant.TOKEN_BUCKET:
                // 构建令牌桶DTO
                return new TokenBucketDTO(annotation.capacity(), annotation.refillRate());
